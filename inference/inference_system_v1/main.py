...
async def main():
    ...
    try:
        ...
        if not await orchestrator.start_classifier_system(SAME_CLASS_INTERVAL, CHECK_INTERVAL):
            return

        print("\nSYSTEM READY!")
        print("The system will now automatically detect and sort bricks!")
        print(f"Same class processing interval: {SAME_CLASS_INTERVAL}s")
        ...
        await orchestrator.automatic_brick_sorting_loop(
            ...
            min_processing_interval=SAME_CLASS_INTERVAL  # Same as classifier interval
        )
