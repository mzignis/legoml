...
class SimpleBrickNet(nn.Module):
    def __init__(self, num_classes=13):
        ...
        # Based on the error messages, the saved model has a 3-layer classifier:
        # classifier.1: Linear(4096, 512)
        # classifier.4: Linear(512, 128) 
        # classifier.7: Linear(128, 13)
        self.classifier = nn.Sequential(
            nn.Flatten(),                             # layer 0
            nn.Linear(256 * 4 * 4, 512),            # layer 1: 4096 -> 512
            nn.ReLU(inplace=True),                   # layer 2
            nn.Dropout(0.3),                         # layer 3
            nn.Linear(512, 128),                     # layer 4: 512 -> 128
            nn.ReLU(inplace=True),                   # layer 5
            nn.Dropout(0.3),                         # layer 6
            nn.Linear(128, num_classes)              # layer 7: 128 -> 13
        )
